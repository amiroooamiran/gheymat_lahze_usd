    ...
    while True:
        ...
        if price_ceiling is not None and price_floor is not None and traded_price is not None:
            if market_open and (previous_market_open is None or not previous_market_open):
                message = f"Market is now open. Ceiling price: {price_ceiling}, Floor price: {price_floor}, Traded price: {traded_price} | Time: {time_of_dollar}"
                send_message(bot_token, channel_id, message)
                print("Message sent:", message)
            elif not market_open and (previous_market_open is None or previous_market_open) and not market_closed_notified:
                message = "Market is now closed."
                send_message(bot_token, channel_id, message)
                print("Message sent:", message)
                market_closed_notified = True
            else:
                message = f"Ceiling price: {price_ceiling}, Floor price: {price_floor}, Traded price: {traded_price} | Time: {time_of_dollar}"
                send_message(bot_token, channel_id, message)
                print("Message sent:", message)